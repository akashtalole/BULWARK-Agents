...
gcloud run deploy "${SERVICE_NAME}" \
  ...
  --project="${PROJECT_ID}" \
  --region="${REGION}" \
  --set-env-vars="GOOGLE_CLOUD_PROJECT=${PROJECT_ID},GOOGLE_GENAI_USE_VERTEXAI=true,USE_FIRESTORE=true,FIRESTORE_DATABASE=${FIRESTORE_DATABASE:-bulwark},USE_PUBSUB=${USE_PUBSUB:-true},BULWARK_ENVIRONMENT=cloud-run,GEMINI_FLASH_MODEL=${GEMINI_FLASH_MODEL:-gemini-flash-latest},GEMINI_PRO_MODEL=${GEMINI_PRO_MODEL:-gemini-pro-latest},BULWARK_API_KEYS=${BULWARK_API_KEYS:-demo-key}" \
  --min-instances=0 \
